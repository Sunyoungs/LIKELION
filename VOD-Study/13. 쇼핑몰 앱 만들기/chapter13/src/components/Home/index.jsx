...
const Home = () => {
  ...
  if (isProductsLoading) { return <div>상품을 로딩 중입니다...</div>; }
  if (isProductsError) { return <div>상품 목록을 가져오는 중에 에러가 발생했습니다.</div>; }
    ...
    <div>
      <ul className={styles.categoryList}>
        {[ "전체", "상의", "하의", "신발", "가방", "악세서리"].map(
          (category) => {
            return (
              <li key={category} className={selectedCategory === category ? styles.selected : null} onClick={() => { setSelectedCategory(category); }}>{category}</li>
            );
          }
        )}
      </ul>
